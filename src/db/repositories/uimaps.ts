...
function mergeJsonObject(existing: string, updates: Record<string, unknown>): string {
...
export function getUiMap(id: string): UiMapRow | undefined {
  return getDb().prepare("SELECT * FROM ui_maps WHERE id = ?").get(id) as UiMapRow | undefined;
}

export function listUiMaps(): UiMapRow[] {
  return getDb().prepare("SELECT * FROM ui_maps ORDER BY updated_at DESC").all() as UiMapRow[];
}
...
/** Find UI maps that contain a given target URL. */
...
export function resolveUiMap(opts: { ticketId?: string; repoUrl?: string; targetUrl?: string }): UiMapRow | undefined {
  if (opts.ticketId) {
...
export function updateUiMap(id: string, updates: {
  name?: string; description?: string;
  addRepos?: string[]; addTargets?: string[]; addTicketIds?: string[];
}): void {
  const db = getDb();
  const map = db.prepare("SELECT * FROM ui_maps WHERE id = ?").get(id) as UiMapRow | undefined;
  if (!map) return;
  ...
  params.push(id);
  db.prepare(`UPDATE ui_maps SET ${sets.join(", ")} WHERE id = ?`).run(...params);
}
...
export function upsertPage(input: CreatePageInput): string {
  ...
  if (existing) {
    ...
    if (input.pageTitle) { sets.push("page_title = ?"); params.push(input.pageTitle); }
    if (input.description) { sets.push("description = ?"); params.push(input.description); }
    if (input.snapshotPath) { sets.push("snapshot_path = ?"); params.push(input.snapshotPath); }
    if (input.screenshotPath) { sets.push("screenshot_path = ?"); params.push(input.screenshotPath); }
    if (input.authRequired !== undefined) { sets.push("auth_required = ?"); params.push(input.authRequired ? 1 : 0); }
    ...
    if (input.createdByRun) { sets.push("updated_by_run = ?"); params.push(input.createdByRun); }
    if (input.createdByTicket) { sets.push("updated_by_ticket = ?"); params.push(input.createdByTicket); }

    sets.push("last_verified_at = datetime('now')");
    if (input.createdByRun) { sets.push("last_verified_run = ?"); params.push(input.createdByRun); }
    sets.push("status = 'active'");

    params.push(existing.id);
    db.prepare(`UPDATE ui_map_pages SET ${sets.join(", ")} WHERE id = ?`).run(...params);
    return existing.id;
  }
  ...
  db.prepare(
    ...
    id, input.uiMapId, input.urlPattern,
    input.pageTitle ?? null, input.description ?? null,
    input.snapshotPath ?? null, input.screenshotPath ?? null,
...
export function getPage(id: string): UiMapPageRow | undefined {
  return getDb().prepare("SELECT * FROM ui_map_pages WHERE id = ?").get(id) as UiMapPageRow | undefined;
}

export function getPageByUrl(uiMapId: string, urlPattern: string): UiMapPageRow | undefined {
  return getDb().prepare(
    "SELECT * FROM ui_map_pages WHERE ui_map_id = ? AND url_pattern = ?"
...
export function updatePageStatus(pageId: string, status: string, opts?: { runId?: string; ticketId?: string }): void {
  const sets = ["status = ?", "updated_at = datetime('now')"];
  const params: unknown[] = [status];
  if (opts?.runId) { sets.push("updated_by_run = ?"); params.push(opts.runId); }
  if (opts?.ticketId) { sets.push("updated_by_ticket = ?"); params.push(opts.ticketId); }
  params.push(pageId);
  getDb().prepare(`UPDATE ui_map_pages SET ${sets.join(", ")} WHERE id = ?`).run(...params);
}
...
export function upsertElement(input: CreateElementInput): string {
  ...
  const existing = db.prepare(
    "SELECT * FROM ui_map_elements WHERE page_id = ? AND selector = ?"
  ...
  if (existing) {
    ...
    if (input.elementText) { sets.push("element_text = ?"); params.push(input.elementText); }
    if (input.elementRole) { sets.push("element_role = ?"); params.push(input.elementRole); }
    if (input.elementName) { sets.push("element_name = ?"); params.push(input.elementName); }
    if (input.positionHint) { sets.push("position_hint = ?"); params.push(input.positionHint); }
    if (input.actionType) { sets.push("action_type = ?"); params.push(input.actionType); }
    if (input.actionResult) { sets.push("action_result = ?"); params.push(input.actionResult); }
    if (input.relatedCode) { sets.push("related_code = ?"); params.push(input.relatedCode); }
    if (input.relatedRepos?.length) {
      sets.push("related_repos = ?");
      params.push(mergeJsonArray(existing.related_repos, input.relatedRepos));
    }
    ...
    params.push(existing.id);
    db.prepare(`UPDATE ui_map_elements SET ${sets.join(", ")} WHERE id = ?`).run(...params);
    return existing.id;
  }
  ...
  db.prepare(
    ...
    id, input.pageId, input.uiMapId, input.selector, input.elementType,
    input.elementRole ?? null, input.elementText ?? null, input.elementName ?? null,
    input.positionHint ?? null, input.actionType ?? null, input.actionResult ?? null,
    input.relatedCode ?? null,
    JSON.stringify(input.relatedRepos ?? []),
    JSON.stringify(input.ticketIds ?? []),
    JSON.stringify(input.authRoles ?? []),
    JSON.stringify(input.targetParity ?? {}),
    input.createdByRun ?? null, input.createdBySession ?? null,
    input.createdByTicket ?? null, input.createdByTestcase ?? null
  );
  return id;
}
...
export function addAltSelector(elementId: string, selector: string): void {
  const db = getDb();
  const el = db.prepare("SELECT alt_selectors FROM ui_map_elements WHERE id = ?").get(elementId) as { alt_selectors: string } | undefined;
  if (!el) return;
  const updated = mergeJsonArray(el.alt_selectors, [selector]);
  db.prepare("UPDATE ui_map_elements SET alt_selectors = ?, updated_at = datetime('now') WHERE id = ?").run(updated, elementId);
}

export function updateElementStatus(elementId: string, status: string, opts?: { runId?: string; ticketId?: string }): void {
  const sets = ["status = ?", "updated_at = datetime('now')"];
  const params: unknown[] = [status];
  if (opts?.runId) { sets.push("updated_by_run = ?"); params.push(opts.runId); }
  if (opts?.ticketId) { sets.push("updated_by_ticket = ?"); params.push(opts.ticketId); }
  params.push(elementId);
  getDb().prepare(`UPDATE ui_map_elements SET ${sets.join(", ")} WHERE id = ?`).run(...params);
}
...
export function lookupElements(uiMapId: string, urlPattern: string, elementType?: string): UiMapElementRow[] {
  const db = getDb();
...
export function upsertNavigation(input: CreateNavInput): string {
  ...
  ).get(input.uiMapId, input.fromPageId, input.toPageId, input.viaElementId ?? "") as UiMapNavigationRow | undefined;

  if (existing) {
    const sets: string[] = ["updated_at = datetime('now')", "times_used = times_used + 1"];
    const params: unknown[] = [];
    if (input.ticketIds?.length) {
      sets.push("ticket_ids = ?");
      params.push(mergeJsonArray(existing.ticket_ids, input.ticketIds));
    ...
    if (input.createdByTicket) { sets.push("updated_by_ticket = ?"); params.push(input.createdByTicket); }
    params.push(existing.id);
    db.prepare(`UPDATE ui_map_navigations SET ${sets.join(", ")} WHERE id = ?`).run(...params);
    return existing.id;
  }
  ...
  db.prepare(
    ...
    id, input.uiMapId, input.fromPageId, input.toPageId,
    input.viaElementId ?? null, input.navType ?? "click",
    JSON.stringify(input.conditions ?? []),
    JSON.stringify(input.ticketIds ?? []),
    JSON.stringify(input.authRoles ?? []),
    JSON.stringify(input.targetParity ?? {}),
    input.createdByRun ?? null, input.createdByTicket ?? null
  );
  return id;
}
...
export function findPath(uiMapId: string, fromUrl: string, toUrl: string): Array<{ page: UiMapPageRow; nav?: UiMapNavigationRow; element?: UiMapElementRow }> | null {
  const db = getDb();
  const fromPage = db.prepare("SELECT * FROM ui_map_pages WHERE ui_map_id = ? AND url_pattern = ?").get(uiMapId, normalizeUrl(fromUrl)) as UiMapPageRow | undefined;
  const toPage = db.prepare("SELECT * FROM ui_map_pages WHERE ui_map_id = ? AND url_pattern = ?").get(uiMapId, normalizeUrl(toUrl)) as UiMapPageRow | undefined;
  ...
  const queue: Array<{ pageId: string; path: Array<{ pageId: string; navId?: string }> }> = [
  ...
  while (queue.length > 0) {
    ...
    if (pageId === toPage.id) {
      ...
      return path.map(step => {
        const page = db.prepare("SELECT * FROM ui_map_pages WHERE id = ?").get(step.pageId) as UiMapPageRow;
        let nav: UiMapNavigationRow | undefined;
        let element: UiMapElementRow | undefined;
        if (step.navId) {
          nav = db.prepare("SELECT * FROM ui_map_navigations WHERE id = ?").get(step.navId) as UiMapNavigationRow;
          if (nav?.via_element_id) {
            element = db.prepare("SELECT * FROM ui_map_elements WHERE id = ?").get(nav.via_element_id) as UiMapElementRow;
          }
        }
    ...
    for (const nav of navs) {
      if (nav.from_page_id === pageId && !visited.has(nav.to_page_id)) {
        ...
        queue.push({ pageId: nav.to_page_id, path: [...path, { pageId: nav.to_page_id, navId: nav.id }] });
      }
    }
  }
...
export function upsertForm(input: CreateFormInput): string {
  ...
  ).get(input.pageId, input.formSelector ?? input.formName ?? "") as UiMapFormRow | undefined;

  if (existing) {
    const sets: string[] = ["updated_at = datetime('now')"];
    const params: unknown[] = [];

    if (input.fields) { sets.push("fields = ?"); params.push(JSON.stringify(input.fields)); }
    if (input.submitElementId) { sets.push("submit_element_id = ?"); params.push(input.submitElementId); }
    if (input.successIndicator) { sets.push("success_indicator = ?"); params.push(input.successIndicator); }
    if (input.errorIndicator) { sets.push("error_indicator = ?"); params.push(input.errorIndicator); }
    ...
    params.push(existing.id);
    db.prepare(`UPDATE ui_map_forms SET ${sets.join(", ")} WHERE id = ?`).run(...params);
    return existing.id;
  }
  ...
  db.prepare(
    ...
    id, input.pageId, input.uiMapId,
    input.formSelector ?? null, input.formName ?? null,
    JSON.stringify(input.fields ?? []),
    input.submitElementId ?? null,
    input.successIndicator ?? null, input.errorIndicator ?? null,
...
export function getMapStats(uiMapId: string) {
  const db = getDb();
  const pages = (db.prepare("SELECT COUNT(*) as c FROM ui_map_pages WHERE ui_map_id = ?").get(uiMapId) as { c: number }).c;
  const elements = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ?").get(uiMapId) as { c: number }).c;
  const navs = (db.prepare("SELECT COUNT(*) as c FROM ui_map_navigations WHERE ui_map_id = ?").get(uiMapId) as { c: number }).c;
  const forms = (db.prepare("SELECT COUNT(*) as c FROM ui_map_forms WHERE ui_map_id = ?").get(uiMapId) as { c: number }).c;
  const working = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'working'").get(uiMapId) as { c: number }).c;
  const flaky = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'flaky'").get(uiMapId) as { c: number }).c;
  const broken = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'broken'").get(uiMapId) as { c: number }).c;
  const ticketIds = db.prepare("SELECT DISTINCT value FROM ui_map_pages, json_each(ui_map_pages.ticket_ids) WHERE ui_map_id = ?").all(uiMapId) as Array<{ value: string }>;

  return { pages, elements, navigations: navs, forms, working, flaky, broken, ticketIds: ticketIds.map(j => j.value) };
}
...
const ROLE_MAP: Record<string, string> = {
  link: "link", button: "button", textbox: "input", combobox: "select",
  checkbox: "checkbox", radio: "radio", tab: "tab", menuitem: "menu",
  img: "image", heading: "text", searchbox: "input", slider: "input",
  spinbutton: "input", switch: "checkbox", textarea: "input",
};
...
export function parseSnapshot(snapshotText: string): ParsedElement[] {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (["document", "banner", "navigation", "list", "listitem", "search", "separator", "complementary", "main", "contentinfo", "region"].includes(role)) continue;
    ...
    for (let i = lineIdx + 1; i < Math.min(lineIdx + 5, lines.length); i++) {
      ...
      if (!nextLine || (nextLine.startsWith("- ") && nextLine.match(/\[ref=/))) break;
    ...
    elements.push({
      type, role, text: text || placeholder || title || altText || "",
      selector, selectorType, ref,
      url, placeholder, disabled,
    });
  }
...
export function scanSnapshotIntoPage(
  ...
  for (const el of parsed) {
    ...
    upsertElement({
      ...
      actionType: el.type === "button" || el.type === "link" ? "click"
        : el.type === "input" ? "type"
        : el.type === "select" ? "select"
        : el.type === "checkbox" || el.type === "radio" ? "toggle"
        : undefined,
  ...
  const inputs = parsed.filter(e => ["input", "select", "checkbox", "radio"].includes(e.type));
  ...
  if (inputs.length >= 2 && buttons.length > 0) {
    ...
    const submitKeywords = ["submit", "create", "save", "sign in", "log in", "login", "send", "confirm", "ok", "apply", "add", "update"];
    let submitBtn = buttons.find(b => submitKeywords.some(kw => b.text.toLowerCase().includes(kw)));
    if (!submitBtn) submitBtn = buttons[buttons.length - 1];

    // Get element IDs for the form fields
    const db = getDb();
    const fields = inputs.map(inp => {
      const elRow = db.prepare(
        "SELECT id FROM ui_map_elements WHERE page_id = ? AND selector = ?"
      ).get(pageId, inp.selector) as { id: string } | undefined;
      return {
        elementId: elRow?.id ?? "",
        inputType: inp.type === "select" ? "select" : inp.placeholder?.toLowerCase().includes("password") ? "password" : inp.placeholder?.toLowerCase().includes("email") ? "email" : "text",
        label: inp.text || inp.placeholder || inp.type,
      };
    }).filter(f => f.elementId);

    const submitRow = db.prepare(
      "SELECT id FROM ui_map_elements WHERE page_id = ? AND selector = ?"
