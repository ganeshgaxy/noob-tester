...
import { getResourceStatsFromCache, refreshAllStats, getStat } from "../db/repositories/resource-stats.js";
...
export function startWatchServer(opts: WatchOptions): void {
  ...
  const server = createServer((req, res) => {
    ...
    if (url.pathname === "/api/secrets" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          if (!target || !key || !value) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "target, key, and value required" }));
            return;
          }
    ...
    if (url.pathname === "/api/secrets/import-op" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          const imported = importFromOnePassword(opRef, target, role ?? "default", {
            live: live ?? false,
          });
          res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/runpacks" && req.method === "GET") {
      ...
      if (packId) {
        ...
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Provide ?ticket=<id> or ?pack=<runPackId>" }));
      }
      return;
    }
    ...
    if (url.pathname === "/api/issues/by-ticket" && req.method === "GET") {
      ...
      if (!ticket) { res.writeHead(400); res.end('{"error":"ticket required"}'); return; }
      const db = getDb();
    ...
    if (url.pathname === "/api/issues/delete" && req.method === "DELETE") {
      ...
      if (!issueId) { res.writeHead(400); res.end('{"error":"id required"}'); return; }
      const db = getDb();
    ...
    if (url.pathname === "/api/issues/detail" && req.method === "GET") {
      ...
      if (!issue) { res.writeHead(404); res.end('{"error":"not found"}'); return; }

      // Related run
      const run = issue.run_id ? db.prepare("SELECT * FROM runs WHERE id = ?").get(issue.run_id) : null;

      // Related test step
      const step = issue.step_id ? db.prepare("SELECT * FROM test_steps WHERE id = ?").get(issue.step_id) : null;

      // Related run pack entry (find by run_id)
      const runpackEntry = issue.run_id ? db.prepare(
      ...
      ).get(issue.run_id) : null;

      // Related analyses for this run
      const analyses = issue.run_id ? db.prepare(
      ...
      if (issue.run_id) {
        ...
        for (const e of entries) {
          try {
            const parsed = JSON.parse(e.artifacts) as Array<Record<string, unknown>>;
            // Keep artifacts that match the issue's location or have no location filter
            if (issue.location) {
              const loc = issue.location as string;
              artifacts = artifacts.concat(parsed.filter(a =>
                !a.path || (a.label && String(a.label).toLowerCase().includes(loc.split("/").pop()?.toLowerCase() || ""))
              ));
        ...
        if (issue.location) {
          const loc = issue.location as string;
          const urlPart = loc.startsWith("http") ? (() => { try { return new URL(loc).pathname; } catch { return loc; } })() : loc;
          runArtifacts = db.prepare(
      ...
      if (uimapPage) {
        ...
        const allPages = db.prepare("SELECT * FROM ui_map_pages WHERE ui_map_id = ?").all(mapId);
        const allNavs = db.prepare("SELECT * FROM ui_map_navigations WHERE ui_map_id = ?").all(mapId);
        const allElements = db.prepare("SELECT * FROM ui_map_elements WHERE ui_map_id = ?").all(mapId);
        uimapFull = { pages: allPages, navigations: allNavs, elements: allElements, highlightPageId: (uimapPage as Record<string, unknown>).id };
      }

      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ issue, run, step, runpackEntry, analyses, techIssues, uimapPage, uimapElements, artifacts, runArtifacts, uimapFull }));
      return;
    }
    ...
    if (url.pathname === "/api/testcases" && req.method === "GET") {
      ...
      let cases: unknown[];
      if (ticket) {
        cases = db.prepare("SELECT * FROM test_cases WHERE ticket_ref = ? ORDER BY priority, created_at").all(ticket);
      } else if (runId) {
        cases = db.prepare("SELECT * FROM test_cases WHERE run_id = ? ORDER BY priority, created_at").all(runId);
      } else {
        cases = db.prepare("SELECT * FROM test_cases ORDER BY priority, created_at LIMIT 100").all();
      }
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/testcases/delete" && req.method === "DELETE") {
      ...
      if (!ticket) { res.writeHead(400); res.end('{"error":"ticket required"}'); return; }
      const db = getDb();
      const result = db.prepare("DELETE FROM test_cases WHERE ticket_ref = ?").run(ticket);
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/testcases/stats" && req.method === "GET") {
      ...
      const byStatus = db.prepare(
        "SELECT status, COUNT(*) as c FROM test_cases GROUP BY status"
      ).all() as Array<{ status: string; c: number }>;
      const byType = db.prepare(
        "SELECT type, COUNT(*) as c FROM test_cases GROUP BY type"
      ...
      const total = (db.prepare("SELECT COUNT(*) as c FROM test_cases").get() as { c: number }).c;
      const tickets = db.prepare(
    ...
    if (url.pathname === "/api/report" && req.method === "GET") {
      const ticketId = url.searchParams.get("ticket");
      if (!ticketId) { res.writeHead(400); res.end('{"error":"ticket required"}'); return; }
      try {
    ...
    if (url.pathname === "/api/report/saved" && req.method === "GET") {
      ...
      const db = getDb();
      const reports = db
        .prepare("SELECT id, ticket_id, run_id, verdict, summary, analysis, improvements, created_at FROM reports WHERE ticket_id = ? ORDER BY created_at DESC")
        .all(ticketId);
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/repos/delete" && req.method === "DELETE") {
      ...
      if (!name) { res.writeHead(400); res.end('{"error":"name required"}'); return; }
      const db = getDb();
      const repo = db.prepare("SELECT id, local_path FROM repos WHERE name = ?").get(name) as { id: string; local_path: string | null } | undefined;
      if (!repo) { res.writeHead(404); res.end('{"error":"not found"}'); return; }
      // Delete DB entries
      ...
      if (existsSync(repoPath)) {
        try { rmSync(repoPath, { recursive: true, force: true }); localDeleted = true; } catch {}
      }
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/repos" && req.method === "GET") {
      const db = getDb();
      const repos = db.prepare("SELECT * FROM repos ORDER BY name").all() as Array<{
      ...
      if (needsRefresh) {
        const liveStats = repos.map((r) => {
          let ftsCount = 0, importCount = 0;
          try { ftsCount = (db.prepare("SELECT COUNT(DISTINCT file_path) as c FROM code_fts WHERE repo_name = ?").get(r.name) as { c: number }).c; } catch {}
          try { importCount = (db.prepare("SELECT COUNT(*) as c FROM import_graph WHERE repo_name = ?").get(r.name) as { c: number }).c; } catch {}
          // Populate the cache for next time
          try {
            db.prepare("INSERT INTO resource_stats (key, value, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = datetime('now')").run(`repo:${r.name}:indexed_files`, String(ftsCount));
            db.prepare("INSERT INTO resource_stats (key, value, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = datetime('now')").run(`repo:${r.name}:indexed_imports`, String(importCount));
          } catch {}
          return { ...r, indexed_files: ftsCount, indexed_imports: importCount };
        });
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ repos: reposWithStats, groups: groupsWithMembers }));
      return;
    }
    ...
    if (url.pathname === "/api/analyses" && req.method === "GET") {
      ...
      if (runId) {
        const analyses = db.prepare("SELECT * FROM analyses WHERE run_id = ? ORDER BY created_at").all(runId);
        res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/metrics" && req.method === "GET") {
      ...
      const testcases = (db.prepare("SELECT COUNT(*) as c FROM test_cases").get() as { c: number }).c;
      const totalIssues = (db.prepare("SELECT COUNT(*) as c FROM issues").get() as { c: number }).c;
    ...
    if (url.pathname === "/api/metrics/resources" && req.method === "GET") {
      ...
      if (!needsRefresh) {
        const db2 = getDb();
        const reposWithPaths = db2.prepare("SELECT name, local_path FROM repos WHERE local_path IS NOT NULL").all() as Array<{ name: string; local_path: string }>;
        for (const r of reposWithPaths) {
          const cached = getStat(`repo:${r.name}:disk_bytes`);
          if (cached === null) { needsRefresh = true; break; } // key never set
        }
      }
      if (needsRefresh) {
        try { refreshAllStats(); } catch {}
      }

      const cached = getResourceStatsFromCache();
      const reposTotal = cached.repos.repos.reduce((s, r) => ({ bytes: s.bytes + Math.max(0, r.bytes), fileCount: s.fileCount + r.fileCount }), { bytes: 0, fileCount: 0 });
      ...
      res.end(JSON.stringify({
        ...
        repos: { bytes: reposTotal.bytes, fileCount: reposTotal.fileCount, repos: cached.repos.repos },
        index: cached.index,
    ...
    if (url.pathname === "/api/uimaps" && req.method === "GET") {
      const db = getDb();
      const maps = db.prepare("SELECT * FROM ui_maps ORDER BY updated_at DESC").all() as Array<Record<string, unknown>>;
      // Add stats to each map
      const result = maps.map(m => {
        const pages = (db.prepare("SELECT COUNT(*) as c FROM ui_map_pages WHERE ui_map_id = ?").get(m.id) as { c: number }).c;
        const elements = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ?").get(m.id) as { c: number }).c;
        const navs = (db.prepare("SELECT COUNT(*) as c FROM ui_map_navigations WHERE ui_map_id = ?").get(m.id) as { c: number }).c;
        const forms = (db.prepare("SELECT COUNT(*) as c FROM ui_map_forms WHERE ui_map_id = ?").get(m.id) as { c: number }).c;
        const working = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'working'").get(m.id) as { c: number }).c;
        const flaky = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'flaky'").get(m.id) as { c: number }).c;
        const broken = (db.prepare("SELECT COUNT(*) as c FROM ui_map_elements WHERE ui_map_id = ? AND status = 'broken'").get(m.id) as { c: number }).c;
        return { ...m, stats: { pages, elements, navigations: navs, forms, working, flaky, broken } };
      });
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/uimaps/delete" && req.method === "DELETE") {
      ...
      if (!mapId) { res.writeHead(400); res.end('{"error":"id required"}'); return; }
      const db = getDb();
      const pages = db.prepare("SELECT id FROM ui_map_pages WHERE ui_map_id = ?").all(mapId) as Array<{ id: string }>;
      ...
      db.prepare("DELETE FROM ui_map_navigations WHERE ui_map_id = ?").run(mapId);
    ...
    if (url.pathname === "/api/uimaps/page/delete" && req.method === "DELETE") {
      ...
      if (!pageId) { res.writeHead(400); res.end('{"error":"id required"}'); return; }
      const db = getDb();
      db.prepare("DELETE FROM ui_map_forms WHERE page_id = ?").run(pageId);
      db.prepare("DELETE FROM ui_map_navigations WHERE from_page_id = ? OR to_page_id = ?").run(pageId, pageId);
      db.prepare("DELETE FROM ui_map_elements WHERE page_id = ?").run(pageId);
      const result = db.prepare("DELETE FROM ui_map_pages WHERE id = ?").run(pageId);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ deleted: result.changes > 0 }));
      return;
    }
    ...
    if (url.pathname === "/api/uimaps/detail" && req.method === "GET") {
      ...
      const pages = db.prepare("SELECT * FROM ui_map_pages WHERE ui_map_id = ? ORDER BY url_pattern").all(mapId) as Array<Record<string, unknown>>;
      const elements = db.prepare("SELECT * FROM ui_map_elements WHERE ui_map_id = ? ORDER BY page_id, element_type").all(mapId);
      const navs = db.prepare("SELECT * FROM ui_map_navigations WHERE ui_map_id = ? ORDER BY from_page_id").all(mapId);
      const forms = db.prepare("SELECT * FROM ui_map_forms WHERE ui_map_id = ? ORDER BY page_id").all(mapId);
      ...
      for (const p of pages) {
        ...
        ).all("%" + urlPattern + "%") as Array<{ severity: string; c: number }>;
        p.issue_counts = Object.fromEntries(issues.map(i => [i.severity, i.c]));
        p.total_issues = issues.reduce((s, i) => s + i.c, 0);

        // Tech issues
        const techIssues = db.prepare(
          "SELECT status, COUNT(*) as c FROM tech_issues WHERE url LIKE ? GROUP BY status"
        ).all("%" + urlPattern + "%") as Array<{ status: string; c: number }>;
        p.tech_issues = Object.fromEntries(techIssues.map(t => [t.status, t.c]));
        p.has_unresolved_tech = techIssues.some(t => t.status === "unresolved");
        ...
        ).get("%" + urlPattern + "%") as { c: number }).c;
        p.console_errors = consoleErrors;
      }

      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ map, pages, elements, navigations: navs, forms }));
      return;
    }
    ...
    if (url.pathname === "/api/plans/delete" && req.method === "DELETE") {
      ...
      if (!planId) { res.writeHead(400); res.end('{"error":"id required"}'); return; }
      const db = getDb();
      ...
      const result = db.prepare("DELETE FROM test_plans WHERE id = ?").run(planId);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ deleted: result.changes > 0 }));
      return;
    }
    ...
    if (url.pathname === "/api/plans" && req.method === "GET") {
      ...
      if (planId) {
        const plan = db.prepare("SELECT * FROM test_plans WHERE id = ?").get(planId) as any;
        ...
        if (plan?.analysis_run_id) {
          linkedAnalyses = db.prepare("SELECT * FROM analyses WHERE run_id = ?").all(plan.analysis_run_id);
        }
        // Include normalized coverage gaps and blockers
        const coverageGaps = db.prepare("SELECT * FROM coverage_gaps WHERE plan_id = ? ORDER BY created_at").all(planId);
        const normalizedBlockers = db.prepare("SELECT * FROM blockers WHERE plan_id = ? ORDER BY created_at").all(planId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ plan, steps, linkedAnalyses, coverageGaps, blockers: normalizedBlockers }));
      } else if (ticketId) {
        ...
        res.writeHead(400); res.end('{"error":"ticket or id required"}');
      }
      return;
    }
    ...
    if (url.pathname === "/api/impact-areas" && req.method === "GET") {
      ...
      if (analysisId) {
        const areas = db.prepare("SELECT * FROM impact_areas WHERE analysis_id = ? ORDER BY severity, area_type").all(analysisId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(areas));
      } else if (runId) {
        const areas = db.prepare("SELECT * FROM impact_areas WHERE run_id = ? ORDER BY severity, area_type").all(runId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(areas));
      } else {
        // Stats view
    ...
    if (url.pathname === "/api/coverage-gaps" && req.method === "GET") {
      ...
      if (planId) {
        const gaps = db.prepare("SELECT * FROM coverage_gaps WHERE plan_id = ? ORDER BY created_at").all(planId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(gaps));
      } else if (runId) {
        const gaps = db.prepare("SELECT * FROM coverage_gaps WHERE run_id = ? ORDER BY created_at").all(runId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(gaps));
      } else {
        const stats = db.prepare(
    ...
    if (url.pathname === "/api/phase-transitions" && req.method === "GET") {
      ...
      if (runId) {
        const transitions = db.prepare("SELECT * FROM phase_transitions WHERE run_id = ? ORDER BY transitioned_at").all(runId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(transitions));
      } else if (sessionId) {
        const transitions = db.prepare("SELECT * FROM phase_transitions WHERE session_id = ? ORDER BY transitioned_at").all(sessionId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(transitions));
      } else {
        const recent = db.prepare("SELECT * FROM phase_transitions ORDER BY transitioned_at DESC LIMIT 50").all();
        res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/apimaps" && req.method === "GET") {
      ...
      if (id) {
        ...
        if (!map) { res.writeHead(404); res.end('{"error":"not found"}'); return; }
        const endpoints = db.prepare("SELECT * FROM api_map_endpoints WHERE api_map_id = ? ORDER BY path, method").all(id);
        const params = db.prepare("SELECT * FROM api_map_params WHERE api_map_id = ?").all(id);
        const responses = db.prepare("SELECT * FROM api_map_responses WHERE api_map_id = ?").all(id);
        const chains = db.prepare("SELECT * FROM api_map_chains WHERE api_map_id = ?").all(id);
        res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/apimaps/endpoint" && req.method === "GET") {
      ...
      const db = getDb();
      const endpoint = db.prepare("SELECT * FROM api_map_endpoints WHERE id = ?").get(id);
      const params = db.prepare("SELECT * FROM api_map_params WHERE endpoint_id = ? ORDER BY location, name").all(id);
      const responses = db.prepare("SELECT * FROM api_map_responses WHERE endpoint_id = ? ORDER BY status_code").all(id);
      const chainsFrom = db.prepare(
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ endpoint, params, responses, chainsFrom, chainsTo }));
      return;
    }

    // ── Ticket Context API ──

    if (url.pathname === "/api/ticket-context/tickets" && req.method === "GET") {
      const db = getDb();
      const tickets = db.prepare(
        `SELECT ticket_id,
    ...
    if (url.pathname === "/api/ticket-context/invalidate" && req.method === "DELETE") {
      const ticketId = url.searchParams.get("ticket");
      ...
      let deleted = 0;
      if (!type) {
        const rows = db.prepare("SELECT file_path FROM ticket_context_index WHERE ticket_id = ?").all(ticketId) as Array<{ file_path: string }>;
        for (const r of rows) { try { unlinkSync(r.file_path); } catch {} }
        deleted = db.prepare("DELETE FROM ticket_context_index WHERE ticket_id = ?").run(ticketId).changes;
      } else {
        // Exact match
        const exact = db.prepare("SELECT id, file_path FROM ticket_context_index WHERE ticket_id = ? AND context_type = ?").get(ticketId, type) as { id: string; file_path: string } | undefined;
        if (exact) {
          ...
          // Prefix match
          const rows = db.prepare("SELECT id, file_path FROM ticket_context_index WHERE ticket_id = ? AND context_type LIKE ?").all(ticketId, type + ":%") as Array<{ id: string; file_path: string }>;
          for (const r of rows) { try { unlinkSync(r.file_path); } catch {} }
          if (rows.length > 0) {
            db.prepare("DELETE FROM ticket_context_index WHERE ticket_id = ? AND context_type LIKE ?").run(ticketId, type + ":%");
          }
          deleted = rows.length;
        }
      }
    ...
    if (url.pathname === "/api/ticket-context/purge" && req.method === "POST") {
      ...
      for (const r of stale) { try { unlinkSync(r.file_path); } catch {} }
      if (stale.length > 0) {
        db.prepare(
    ...
    if (url.pathname === "/api/blockers" && req.method === "GET") {
      ...
      if (planId) {
        const blockers = db.prepare("SELECT * FROM blockers WHERE plan_id = ? ORDER BY created_at").all(planId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(blockers));
      } else if (ticketId) {
        const blockers = db.prepare("SELECT * FROM blockers WHERE ticket_id = ? ORDER BY created_at").all(ticketId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(blockers));
      } else if (runId) {
        const blockers = db.prepare("SELECT * FROM blockers WHERE run_id = ? ORDER BY created_at").all(runId);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(blockers));
      } else {
    ...
    if (url.pathname === "/api/run-artifacts" && req.method === "GET") {
      ...
      if (entryId) {
        artifacts = db.prepare("SELECT * FROM run_artifacts WHERE entry_id = ? ORDER BY action_index, created_at").all(entryId);
      } else if (packId) {
        if (unattachedOnly) {
          artifacts = db.prepare("SELECT * FROM run_artifacts WHERE run_pack_id = ? AND entry_id IS NULL ORDER BY action_index, created_at").all(packId);
        } else {
          artifacts = db.prepare("SELECT * FROM run_artifacts WHERE run_pack_id = ? ORDER BY action_index, created_at").all(packId);
        }
      } else if (runId) {
        if (type) {
          artifacts = db.prepare("SELECT * FROM run_artifacts WHERE run_id = ? AND artifact_type = ? ORDER BY action_index, created_at").all(runId, type);
        } else {
          artifacts = db.prepare("SELECT * FROM run_artifacts WHERE run_id = ? ORDER BY action_index, created_at").all(runId);
        }
      } else {
        res.writeHead(400); res.end('{"error":"run, entry, or pack required"}'); return;
      }
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/coverage/uncovered" && req.method === "GET") {
      ...
      if (!repo) { res.writeHead(400); res.end(JSON.stringify({ error: "repo required" })); return; }
    ...
    if (url.pathname === "/api/coverage/by-file" && req.method === "GET") {
      ...
      if (!repo || !file) { res.writeHead(400); res.end(JSON.stringify({ error: "repo and file required" })); return; }
      const db = getDb();
    ...
    if (url.pathname === "/api/rca/summary" && req.method === "GET") {
      ...
      if (!pack) { res.writeHead(400); res.end(JSON.stringify({ error: "pack required" })); return; }
      const db = getDb();
      const total = (db.prepare("SELECT COUNT(*) as c FROM rca_results WHERE run_pack_id = ?").get(pack) as { c: number }).c;
      const byClass = db.prepare("SELECT classification, COUNT(*) as c FROM rca_results WHERE run_pack_id = ? GROUP BY classification").all(pack);
      const byAction = db.prepare("SELECT suggested_action, COUNT(*) as c FROM rca_results WHERE run_pack_id = ? AND suggested_action IS NOT NULL GROUP BY suggested_action").all(pack);
      const avgConf = (db.prepare("SELECT AVG(confidence) as avg FROM rca_results WHERE run_pack_id = ?").get(pack) as { avg: number | null }).avg;
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ total, byClassification: byClass, byAction, avgConfidence: avgConf ?? 0 }));
      return;
    }
    ...
    if (url.pathname === "/api/a11y/summary" && req.method === "GET") {
      ...
      const pack = url.searchParams.get("pack");
      const ticket = url.searchParams.get("ticket");
      const filter = run ? "run_id = ?" : pack ? "run_pack_id = ?" : ticket ? "ticket_id = ?" : "1=1";
      const params = run ? [run] : pack ? [pack] : ticket ? [ticket] : [];
      const total = (db.prepare(`SELECT COUNT(*) as c FROM a11y_issues WHERE ${filter}`).get(...params) as { c: number }).c;
      const byImpact = db.prepare(`SELECT impact, COUNT(*) as c FROM a11y_issues WHERE ${filter} GROUP BY impact ORDER BY CASE impact WHEN 'critical' THEN 1 WHEN 'serious' THEN 2 WHEN 'moderate' THEN 3 WHEN 'minor' THEN 4 END`).all(...params);
      const byRule = db.prepare(`SELECT rule_id, COUNT(*) as count, impact FROM a11y_issues WHERE ${filter} GROUP BY rule_id ORDER BY count DESC LIMIT 20`).all(...params);
      const pageCount = (db.prepare(`SELECT COUNT(DISTINCT page_url) as c FROM a11y_issues WHERE ${filter}`).get(...params) as { c: number }).c;
      // Tickets list (for top-level view)
      const tickets = db.prepare(`SELECT COALESCE(ticket_id, 'unlinked') as ticket_id, COUNT(*) as issue_count,
        ...
        FROM a11y_issues GROUP BY COALESCE(ticket_id, 'unlinked') ORDER BY critical DESC, issue_count DESC`).all();
      // Run packs for a ticket (level 2)
      const packs = ticket ? db.prepare(`SELECT run_pack_id, COUNT(*) as issue_count,
        SUM(CASE WHEN impact = 'critical' THEN 1 ELSE 0 END) as critical,
        SUM(CASE WHEN impact = 'serious' THEN 1 ELSE 0 END) as serious,
        COUNT(DISTINCT page_url) as pages,
        MIN(created_at) as first_seen, MAX(created_at) as last_seen
        FROM a11y_issues WHERE ticket_id = ?
        GROUP BY run_pack_id ORDER BY MAX(created_at) DESC`).all(ticket) : [];
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ total, byImpact, byRule, pageCount, tickets, packs }));
      return;
    }
    ...
    if (url.pathname === "/api/a11y/issues" && req.method === "GET") {
      ...
      if (run) { conditions.push("run_id = ?"); params.push(run); }
      if (pack) { conditions.push("run_pack_id = ?"); params.push(pack); }
      if (page) { conditions.push("page_url = ?"); params.push(page); }
      if (ticket) { conditions.push("ticket_id = ?"); params.push(ticket); }
      const filter = conditions.length > 0 ? conditions.join(" AND ") : "1=1";
      const issues = db.prepare(`SELECT * FROM a11y_issues WHERE ${filter} ORDER BY CASE impact WHEN 'critical' THEN 1 WHEN 'serious' THEN 2 WHEN 'moderate' THEN 3 WHEN 'minor' THEN 4 END, rule_id LIMIT 200`).all(...params);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify(issues));
      return;
    }
    ...
    if (url.pathname === "/api/false-positives/stats" && req.method === "GET") {
      ...
      if (!pack) { res.writeHead(400); res.end(JSON.stringify({ error: "pack required" })); return; }
      const db = getDb();
      const totalFailed = (db.prepare("SELECT COUNT(*) as c FROM run_pack_entries WHERE run_pack_id = ? AND (status = 'failed' OR original_status = 'failed') AND test_case_id != '__header__'").get(pack) as { c: number }).c;
      const retried = (db.prepare("SELECT COUNT(*) as c FROM run_pack_entries WHERE run_pack_id = ? AND retry_count > 0 AND test_case_id != '__header__'").get(pack) as { c: number }).c;
      const falsePositives = (db.prepare("SELECT COUNT(*) as c FROM run_pack_entries WHERE run_pack_id = ? AND is_false_positive = 1 AND test_case_id != '__header__'").get(pack) as { c: number }).c;
      const byConfidence = db.prepare("SELECT failure_confidence, COUNT(*) as c FROM run_pack_entries WHERE run_pack_id = ? AND failure_confidence IS NOT NULL AND test_case_id != '__header__' GROUP BY failure_confidence").all(pack);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ totalFailed, retried, falsePositives, confirmedFailures: totalFailed - falsePositives, byConfidence }));
      return;
    }
    ...
    if (url.pathname === "/api/test-audit" && req.method === "GET") {
      ...
      const testCases = db.prepare(`SELECT id, title, description, type, bdd_scenario, trad_steps FROM test_cases${filter}`).all(...params) as Array<{ id: string; title: string; description: string | null; type: string; bdd_scenario: string | null; trad_steps: string | null }>;

      const tokenized = testCases.map(tc => {
        const text = [tc.title, tc.description ?? "", tc.bdd_scenario ?? "", tc.trad_steps ?? ""].join(" ").toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter(w => w.length > 2);
        return { tc, tokens: new Set(text) };
      });

      const duplicates: Array<{ a: { id: string; title: string; type: string }; b: { id: string; title: string; type: string }; similarity: number }> = [];
      for (let i = 0; i < tokenized.length && i < 100; i++) {
        for (let j = i + 1; j < tokenized.length && j < 100; j++) {
          ...
          if (sim >= 0.65) duplicates.push({ a: { id: a.tc.id, title: a.tc.title, type: a.tc.type }, b: { id: b.tc.id, title: b.tc.title, type: b.tc.type }, similarity: Math.round(sim * 100) / 100 });
        }
      }
      ...
      const orphaned = ticket ? [] : db.prepare(`
      ...
      const neverExecuted = db.prepare(`
      ...
      const total = (db.prepare(`SELECT COUNT(*) as c FROM test_cases${filter}`).get(...params) as { c: number }).c;
      ...
      res.end(JSON.stringify({
        ...
        duplicates, neverFailed, orphaned, stale, neverExecuted
      }));
      return;
    }
    ...
    if (url.pathname === "/api/visual-regression/stats" && req.method === "GET") {
      const db = getDb();
      const run = url.searchParams.get("run");
      const filter = run ? " WHERE run_id = ?" : "";
      const params = run ? [run] : [];
      const totalDiffs = (db.prepare(`SELECT COUNT(*) as c FROM visual_diffs${filter}`).get(...params) as { c: number }).c;
      const regressions = (db.prepare(`SELECT COUNT(*) as c FROM visual_diffs WHERE is_regression = 1${run ? " AND run_id = ?" : ""}`).get(...params) as { c: number }).c;
      const reviewed = (db.prepare(`SELECT COUNT(*) as c FROM visual_diffs WHERE reviewed = 1${run ? " AND run_id = ?" : ""}`).get(...params) as { c: number }).c;
      const totalBaselines = (db.prepare("SELECT COUNT(*) as c FROM visual_baselines WHERE status = 'active'").get() as { c: number }).c;
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ totalDiffs, regressions, reviewed, unreviewed: totalDiffs - reviewed, totalBaselines }));
      return;
    }

    if (url.pathname === "/api/visual-regression/baselines" && req.method === "GET") {
      const db = getDb();
    ...
    if (url.pathname === "/api/visual-regression/diffs" && req.method === "GET") {
      const db = getDb();
      const run = url.searchParams.get("run");
      ...
      const conditions: string[] = [];
      const params: unknown[] = [];
      if (run) { conditions.push("vd.run_id = ?"); params.push(run); }
      if (unreviewed) { conditions.push("vd.reviewed = 0"); }
    ...
    if (url.pathname === "/api/visual-regression/review" && req.method === "POST") {
      let body = "";
      req.on("data", (chunk: string) => (body += chunk));
      req.on("end", () => {
        try {
          ...
          getDb().prepare("UPDATE visual_diffs SET reviewed = 1, is_regression = ? WHERE id = ?").run(isRegression ? 1 : 0, id);
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ ok: true }));
    ...
    if (url.pathname === "/api/visual-regression/accept" && req.method === "POST") {
      let body = "";
      req.on("data", (chunk: string) => (body += chunk));
      req.on("end", () => {
        try {
          ...
          if (!id) { res.writeHead(400); res.end(JSON.stringify({ error: "id required" })); return; }
          const db = getDb();
          const diff = db.prepare("SELECT * FROM visual_diffs WHERE id = ?").get(id) as any;
          if (!diff) { res.writeHead(404); res.end(JSON.stringify({ error: "diff not found" })); return; }
          const baseline = db.prepare("SELECT * FROM visual_baselines WHERE id = ?").get(diff.baseline_id) as any;
          if (!baseline) { res.writeHead(404); res.end(JSON.stringify({ error: "baseline not found" })); return; }
          db.prepare("UPDATE visual_diffs SET reviewed = 1, is_regression = 0 WHERE id = ?").run(id);
          db.prepare("UPDATE visual_baselines SET status = 'superseded', updated_at = datetime('now') WHERE id = ?").run(baseline.id);
          const newId = uuidv4();
          let hash: string | null = null;
          try { hash = createHash("sha256").update(readFileSync(diff.current_path)).digest("hex"); } catch {}
          db.prepare("INSERT INTO visual_baselines (id, ui_map_page_id, url_pattern, viewport, baseline_path, baseline_hash, source_run_id, source_entry_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')").run(newId, baseline.ui_map_page_id, baseline.url_pattern, baseline.viewport, diff.current_path, hash, diff.run_id, diff.entry_id);
          res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/runpacks/delete" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          if (pack) {
            db.prepare("DELETE FROM run_pack_entries WHERE run_pack_id = ?").run(pack);
          } else if (ticket) {
            db.prepare("DELETE FROM run_pack_entries WHERE ticket_id = ?").run(ticket);
          } else {
            res.writeHead(400); res.end(JSON.stringify({ error: "ticket or pack required" })); return;
          }
    ...
    if (url.pathname === "/api/session/delete" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          db.pragma("foreign_keys = OFF");
          // Clear run FK first
          db.prepare("UPDATE sessions SET current_run_id = NULL WHERE id = ?").run(id);
          // Get linked runs
          const runs = db.prepare("SELECT id FROM runs WHERE session_id = ?").all(id) as Array<{ id: string }>;
          const runIds = runs.map(r => r.id);
          if (runIds.length > 0) {
            const ph = runIds.map(() => "?").join(",");
            // Delete in safe order — children before parents, try/catch each
            const deletes = [
              "DELETE FROM rca_results WHERE run_pack_id IN (SELECT DISTINCT run_pack_id FROM run_pack_entries WHERE run_id IN (" + ph + "))",
            ...
            for (const sql of deletes) {
              try { db.prepare(sql).run(...runIds); } catch {}
            }
          }
          // Also delete phase transitions by session
          try { db.prepare("DELETE FROM phase_transitions WHERE session_id = ?").run(id); } catch {}
  ...
  server.on("error", (err: NodeJS.ErrnoException) => {
    if (err.code === "EADDRINUSE") {
      ...
      console.log(chalk.dim(`  Fix: noob-tester cleanup watch --port ${opts.port}`));
      console.log(chalk.dim(`  Or use a different port: noob-tester watch --port ${opts.port + 1}\n`));
      process.exit(1);
    }
    throw err;
  });
  ...
  server.listen(opts.port, () => {
    ...
    console.log(
      chalk.green(`  Dashboard: http://localhost:${opts.port}`)
    );
    if (opts.sessionId) {
...
function gatherState(filterSessionId?: string) {
  ...
  // Mark stale sessions (non-critical — don't crash if DB is busy)
  try {
    db.prepare(
    ...
    // DB busy — skip this cycle, will retry in 2 seconds
  }
  ...
  const runs = db.prepare("SELECT * FROM runs ORDER BY created_at DESC LIMIT 50").all();
...
function gatherSessionDetail(sessionId: string) {
  ...
  const runs = db.prepare("SELECT * FROM runs WHERE session_id = ? ORDER BY created_at DESC").all(sessionId);
  ...
  if (runIds.length > 0) {
    ...
    issues = db.prepare(`SELECT * FROM issues WHERE run_id IN (${placeholders}) ORDER BY created_at DESC`).all(...runIds);
    actions = db.prepare(`SELECT * FROM action_log WHERE run_id IN (${placeholders}) ORDER BY created_at DESC LIMIT 100`).all(...runIds);
    analyses = db.prepare(`SELECT * FROM analyses WHERE run_id IN (${placeholders})`).all(...runIds);
    phaseTransitions = db.prepare(`SELECT * FROM phase_transitions WHERE run_id IN (${placeholders}) ORDER BY transitioned_at`).all(...runIds);
  }

  // Also get phase transitions by session_id directly
  const sessionTransitions = db.prepare("SELECT * FROM phase_transitions WHERE session_id = ? ORDER BY transitioned_at").all(sessionId);
  // Merge, dedup by id
  ...
  return { session, runs, issues, actions, analyses, phaseTransitions: dedupedTransitions };
}
...
function getActionsForRun(runId: string) {
  return getDb()
    .prepare("SELECT * FROM action_log WHERE run_id = ? ORDER BY created_at DESC LIMIT 100")
    .all(runId);
}
