...
import { readdirSync, copyFileSync, existsSync as fsExists, mkdirSync as fsMkdir } from "fs";
import { join as pathJoin, basename } from "path";

program
  .command("evidence-path")
  .description("Print the dedicated evidence directory path (~/.noob-tester/evidence/)")
  .action(() => { console.log(evidenceDir()); });

program
  .command("evidence-migrate")
  .description("Move evidence files from a source folder to ~/.noob-tester/evidence/ and update all DB paths")
  .requiredOption("--from <path>", "Source folder with evidence files")
  .option("--yes", "Skip confirmation")
  .action((opts) => {
    const src = opts.from;
    const dest = evidenceDir();
    if (!fsExists(src)) { console.error("Source not found: " + src); process.exit(1); }

    const files = readdirSync(src).filter(f => !f.startsWith("."));
    if (!opts.yes) {
      console.log(`Will copy ${files.length} files from ${src} to ${dest} and update DB paths.`);
    ...
    for (const f of files) {
      ...
      try { copyFileSync(srcPath, destPath); copied++; } catch {}
    }
    ...
    for (const { table, cols } of tables) {
      for (const col of cols) {
        if (col === "artifacts") {
          ...
          const rows = db.prepare(`SELECT id, artifacts FROM ${table} WHERE artifacts LIKE ?`).all("%" + src + "%") as Array<{ id: string; artifacts: string }>;
          for (const row of rows) {
            const newArtifacts = row.artifacts.replace(new RegExp(src.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), dest);
            db.prepare(`UPDATE ${table} SET artifacts = ? WHERE id = ?`).run(newArtifacts, row.id);
            updated++;
          }
        } else {
          const result = db.prepare(`UPDATE ${table} SET ${col} = REPLACE(${col}, ?, ?) WHERE ${col} LIKE ?`).run(src, dest, "%" + src + "%");
