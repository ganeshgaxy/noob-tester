...
export function registerReportCommands(program: Command): void {
  program
    .command("report")
    .description(
      "Generate a comprehensive report for a ticket (all data) or a single run",
    )
    .option(
      "--ticket <id>",
      "Ticket ID — pulls analyses, plans, test cases, run packs, issues, UI maps",
    )
    .option("--run <runId>", "Single run ID (legacy mode — issues only)")
    .option(
      "--json",
      "Output as structured JSON (for /noob-report skill to analyze)",
    )
    .action((opts) => {
      if (opts.ticket) {
  ...
  program
    ...
    .requiredOption(
      "--analysis <text>",
      "Claude's full written analysis — findings, root causes, correlations, what went wrong and why",
    )
    .option(
      "--improvements <text>",
      "Claude's improvement plan — prioritized recommendations",
    )
  ...
  program
    ...
    .action((opts) => {
      ...
      if ((reports as unknown[]).length === 0) {
        console.log(
          "No reports saved for " +
            opts.ticket +
            ". Run /noob-report to generate one.",
        );
        return;
      }
      ...
      for (const r of reports as Array<Record<string, unknown>>) {
        ...
        console.log(
          chalk.dim(`  ${r.created_at}  ${(r.id as string).slice(0, 8)}\n`),
        );
...
function generateTicketReport(ticketId: string, asJson: boolean): void {
  const {
    ...
    latestPlan,
    planSteps,
  } = buildTicketReportData(ticketId);
...
function buildTicketReportData(ticketId: string) {
  ...
  if (runIds.length > 0) {
    ...
    analyses = db
      .prepare(
        `SELECT a.*, r.input_ref as ticket FROM analyses a JOIN runs r ON a.run_id = r.id WHERE a.run_id IN (${ph}) ORDER BY a.analysis_type, a.created_at DESC`,
      )
      .all(...runIds);
  }
  ...
  if (plans.length > 0) {
    const detail = getPlanDetail(plans[0].id as string) as {
  ...
  const testCases = db
    .prepare(
      "SELECT id, title, type, format, status, COALESCE(test_layer, 'ui') as test_layer, ready, execution_count, last_executed FROM test_cases WHERE ticket_ref = ? ORDER BY priority, created_at",
    )
    .all(ticketId);

  // ── 5. Run Packs (execution results) ──
  const runPacks = getRunPacksByTicket(ticketId) as Array<
  ...
  if (runIds.length > 0) {
    ...
    issues = db
      .prepare(
        `SELECT * FROM issues WHERE run_id IN (${ph}) ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 WHEN 'low' THEN 3 WHEN 'info' THEN 4 END, category`,
      )
      .all(...runIds);
  }
  ...
  for (const i of issues as Array<Record<string, unknown>>) {
    issuesBySeverity[i.severity as string] =
      (issuesBySeverity[i.severity as string] || 0) + 1;
    issuesByCategory[i.category as string] =
      (issuesByCategory[i.category as string] || 0) + 1;
  }
  ...
  if (mapRow) {
    ...
    uiMapPages = db
      .prepare(
        "SELECT id, url_pattern, page_title, status, auth_required FROM ui_map_pages WHERE ui_map_id = ? ORDER BY url_pattern",
      )
      .all(mapRow.id as string);
    uiMapFlakyElements = db
      .prepare(
        "SELECT e.selector, e.element_type, e.status, e.times_used, e.times_failed, p.url_pattern FROM ui_map_elements e JOIN ui_map_pages p ON e.page_id = p.id WHERE e.ui_map_id = ? AND e.status IN ('flaky', 'broken') ORDER BY e.times_failed DESC",
      )
      .all(mapRow.id as string);
    const totalElements = (
      db
    ...
    uiMapStats = {
      ...
      totalNavs,
      totalForms,
      flakyCount: uiMapFlakyElements.length,
    };
  }
  ...
  const techIssues = db
    .prepare(
      "SELECT * FROM tech_issues WHERE ticket_ref = ? ORDER BY status, severity DESC",
    )
    .all(ticketId);
  ...
  const sessions = db
    .prepare(
      "SELECT id, task_summary as task, labels, status, created_at, ended_at FROM sessions WHERE ticket_refs LIKE ? ORDER BY created_at DESC",
    )
    .all(`%${ticketId}%`);
  ...
  const report = {
    ...
    analyses: (analyses as Array<Record<string, unknown>>).map((a) => ({
      ...
      content: (() => {
        try {
          return JSON.parse(a.content_json as string);
        } catch {
          return a.content_json;
        }
      })(),
    })),
    ...
    plan: latestPlan
      ? {
          ...
          blockers: (() => {
            try {
              return JSON.parse(
                ((latestPlan as Record<string, unknown>).blockers as string) ||
                  "[]",
              );
            } catch {
              return [];
          ...
          coverageGaps: (() => {
            try {
              return JSON.parse(
                ((latestPlan as Record<string, unknown>)
          ...
          confidentSteps: (planSteps as Array<Record<string, unknown>>).filter(
            (s) => s.confidence === "confident",
          ).length,
          uncertainSteps: (planSteps as Array<Record<string, unknown>>).filter(
    ...
    execution: {
      runPacks: runPacks.map((p, i) => ({
        ...
        entries: (packDetails[i]?.entries || []).map(
          (e: Record<string, unknown>) => ({
            ...
            results: (() => {
              try {
                return JSON.parse((e.results as string) || "null");
              } catch {
                return e.results;
              }
            })(),
            observations: (() => {
              try {
                return JSON.parse((e.observations as string) || "[]");
              } catch {
                return [];
              }
            })(),
            issues: (() => {
              try {
                return JSON.parse((e.issues as string) || "[]");
              } catch {
                return [];
              }
            })(),
...
function printFormattedReport(
  ...
  techIssues: Array<Record<string, unknown>>,
  ...
  console.log(
    chalk.bold(
      `  Verdict: ${insights.verdict === "PASS" ? chalk.green.bold("PASS") : insights.verdict === "FAIL" ? chalk.red.bold("FAIL") : chalk.yellow.bold("PARTIAL")}`,
    ),
  );
  ...
  console.log(chalk.bold("Summary:"));
  console.log(
    `  Runs: ${runs.length}  Sessions: ${(sessions as unknown[]).length}  Issues: ${issues.length}`,
  );
  console.log(
    `  Test Cases: ${testCaseStats.total} (${testCaseStats.ready} ready, ${testCaseStats.draft} draft)`,
  );
  console.log(
    `  Executed: ${totalExecuted} (${totalPassed} passed, ${totalFailed} failed, ${totalBlocked} blocked, ${totalSkipped} skipped)`,
  );
  if (uiExecuted || apiExecuted)
    console.log(`  Runners: ${uiExecuted} UI, ${apiExecuted} API`);
  ...
  if (insights.riskHotspots.length > 0) {
    ...
    for (const h of insights.riskHotspots.slice(0, 5)) {
      console.log(
        chalk.red(`  ${h.location}: ${h.issueCount} issues (${h.severities})`),
      );
    }
    console.log();
  }
  ...
  if (issues.length > 0) {
    ...
    for (const issue of (issues as Array<Record<string, unknown>>).slice(
      0,
      10,
    )) {
      const sevColor =
        issue.severity === "critical"
          ...
          : issue.severity === "high"
            ...
            : issue.severity === "medium"
              ...
              : chalk.dim;
      console.log(
        `  ${sevColor(`[${(issue.severity as string).toUpperCase()}]`)} ${chalk.cyan(`[${issue.category}]`)} ${issue.title}`,
      );
  ...
  if (uiMapStats) {
    ...
    console.log(
      `  Pages: ${s.totalPages}  Elements: ${s.totalElements}  Navigations: ${s.totalNavs}  Forms: ${s.totalForms}`,
    );
    if (s.flakyCount > 0)
      console.log(
        chalk.yellow(
          `  Flaky/Broken: ${s.flakyCount} elements — affects test stability`,
        ),
      );
    console.log();
  }
  ...
  if ((techIssues as unknown[]).length > 0) {
    ...
    for (const ti of (techIssues as Array<Record<string, unknown>>).slice(
      0,
      5,
    )) {
      ...
      const prioColor =
        imp.priority === "critical"
          ...
          : imp.priority === "high"
            ...
            : chalk.dim;
      console.log(
        `  ${i + 1}. ${prioColor(`[${imp.priority.toUpperCase()}]`)} ${imp.action}`,
      );
...
function computeInsights(data: {
  ...
  const directFailed = data.packDetails
    .flatMap((pd) => pd.entries as Array<Record<string, unknown>>)
    .filter(
      (e) => e.status === "failed" && e.tc_type === "direct_functional",
    ).length;
  ...
  if (criticalCount > 0 || directFailed > 0) {
    ...
    if (directFailed > 0)
      reasons.push(
        `${directFailed} direct functional test${directFailed > 1 ? "s" : ""} failed`,
      );
    ...
    if (data.totalFailed > 0)
      reasons.push(
        `${data.totalFailed} test${data.totalFailed > 1 ? "s" : ""} failed`,
      );
  ...
  const passRate =
    data.totalExecuted > 0
      ? Math.round((data.totalPassed / data.totalExecuted) * 100)
      : 0;
  ...
  for (const i of issuesArr) {
    ...
    if (!locationMap[loc])
      locationMap[loc] = { count: 0, severities: new Set() };
  ...
  // Dominant category
  const topCategory = Object.entries(data.issuesByCategory).sort(
    (a, b) => b[1] - a[1],
  )[0];
  if (topCategory && topCategory[1] >= 3) {
    issuePatterns.push(
      `${topCategory[1]} ${topCategory[0]} issues — most common category. Investigate systemic ${topCategory[0]} problems.`,
    );
  }
  ...
  for (const i of issuesArr) {
    titleCounts[i.title as string] = (titleCounts[i.title as string] || 0) + 1;
  }
  const repeatedIssues = Object.entries(titleCounts)
    .filter(([_, c]) => c >= 2)
    .sort((a, b) => b[1] - a[1]);
  for (const [title, count] of repeatedIssues.slice(0, 3)) {
    issuePatterns.push(
      `"${title}" appeared ${count} times — likely a systemic issue, not a one-off`,
    );
  }

  // UI vs API failure comparison
  const uiEntries = data.packDetails
    .flatMap((pd) => pd.entries as Array<Record<string, unknown>>)
    .filter((e) => (e.runner || "ui") === "ui");
  const apiEntries = data.packDetails
    .flatMap((pd) => pd.entries as Array<Record<string, unknown>>)
    .filter((e) => e.runner === "api");
  ...
  if (uiEntries.length > 0 && apiEntries.length > 0) {
    const uiPassRate =
      uiEntries.length > 0
        ? Math.round(
            (uiEntries.filter((e) => e.status === "passed").length /
              uiEntries.length) *
              100,
          )
        : 0;
    const apiPassRate =
      apiEntries.length > 0
        ? Math.round(
            (apiEntries.filter((e) => e.status === "passed").length /
              apiEntries.length) *
              100,
          )
        : 0;
    if (Math.abs(uiPassRate - apiPassRate) > 30) {
      issuePatterns.push(
        `UI pass rate ${uiPassRate}% vs API pass rate ${apiPassRate}% — significant gap between layers`,
      );
    }
  }
  ...
  for (const layer of allLayers) {
    if (!testedLayers.has(layer) && data.testCaseStats.total > 0) {
      coverageGaps.push(
        `No ${layer} layer test cases — ${layer === "api" ? "backend endpoints not directly tested" : layer === "ui_api" ? "UI-to-API integration not verified" : "UI interactions not tested"}`,
      );
    }
  }
  ...
  if (data.testCaseStats.draft > 0) {
    coverageGaps.push(
      `${data.testCaseStats.draft} test case${data.testCaseStats.draft > 1 ? "s" : ""} still in draft — not executed`,
    );
  }
  ...
  if (data.latestPlan) {
    try {
      const blockers = JSON.parse(
        ((data.latestPlan as Record<string, unknown>).blockers as string) ||
          "[]",
      );
      if (Array.isArray(blockers) && blockers.length > 0) {
        coverageGaps.push(
          `Plan has ${blockers.length} blocker${blockers.length > 1 ? "s" : ""}: ${blockers.slice(0, 2).join("; ")}`,
        );
      }
    } catch {}
    try {
      const gaps = JSON.parse(
        ((data.latestPlan as Record<string, unknown>)
          .coverage_gaps as string) || "[]",
      );
      if (Array.isArray(gaps) && gaps.length > 0) {
        for (const g of gaps.slice(0, 3))
          coverageGaps.push(
            `Plan gap: ${typeof g === "string" ? g : JSON.stringify(g)}`,
          );
      }
    } catch {}
  }
  ...
  const uncertainSteps = (
    data.planSteps as Array<Record<string, unknown>>
  ).filter((s) => s.confidence === "uncertain");
  if (uncertainSteps.length > 0) {
    coverageGaps.push(
      `${uncertainSteps.length} plan step${uncertainSteps.length > 1 ? "s" : ""} marked uncertain — may have gaps in test coverage`,
    );
  }
  ...
  if ((data.analyses as unknown[]).length === 0) {
    coverageGaps.push(
      "No analysis was performed — impact, requirements, and feasibility are unknown",
    );
  }

  // ── Analysis accuracy (did impact analysis predict the right areas?) ──
  const analysisAccuracy: string[] = [];
  const impactAnalysis = (data.analyses as Array<Record<string, unknown>>).find(
    (a) => a.analysis_type === "impact",
  );
  if (impactAnalysis) {
    try {
      const content = JSON.parse(impactAnalysis.content_json as string);
      const predictedAreas = (content.impacted_areas || [])
        .map((a: Record<string, unknown>) => a.path || a.area || "")
        .filter(Boolean);
      if (predictedAreas.length > 0 && issuesArr.length > 0) {
        const issueLocations = new Set(
          issuesArr.map((i) => i.location as string).filter(Boolean),
        );
        const predicted = predictedAreas.filter((p: string) =>
          [...issueLocations].some((loc) => loc.includes(p) || p.includes(loc)),
        );
        if (predicted.length > 0) {
          analysisAccuracy.push(
            `Impact analysis correctly predicted ${predicted.length} of ${issueLocations.size} issue locations`,
          );
        }
        const missed = [...issueLocations].filter(
          (loc) =>
            !predictedAreas.some(
              (p: string) => loc.includes(p) || p.includes(loc),
            ),
        );
        if (missed.length > 0) {
          analysisAccuracy.push(
            `${missed.length} issue location${missed.length > 1 ? "s" : ""} not predicted by impact analysis: ${missed.slice(0, 3).join(", ")}`,
          );
        }
      }
    } catch {}
  ...
  if ((data.uiMapFlakyElements as unknown[]).length > 0) {
    testStability.push(
      `${(data.uiMapFlakyElements as unknown[]).length} flaky/broken UI selectors — causes intermittent test failures`,
    );
  }

  // Tech issues
  const unresolvedTech = techArr.filter(
    (t) => t.status !== "resolved" && t.status !== "wont_fix",
  );
  if (unresolvedTech.length > 0) {
    testStability.push(
      `${unresolvedTech.length} unresolved tech issue${unresolvedTech.length > 1 ? "s" : ""} — may affect test reliability`,
    );
  }
  ...
  if (retriedTests.length > 0) {
    testStability.push(
      `${retriedTests.length} test case${retriedTests.length > 1 ? "s" : ""} executed multiple times — indicates instability or reruns`,
    );
  }
  ...
  if (criticalCount > 0) {
    improvements.push({
      priority: "critical",
      action: "Fix all critical issues before release",
      reason: `${criticalCount} critical issue${criticalCount > 1 ? "s" : ""} found`,
    });
  }
  if (highCount > 0) {
    improvements.push({
      ...
      reason: `${highCount} high issue${highCount > 1 ? "s" : ""} — fix before release or document known limitations`,
    });
  }
  if (riskHotspots.length > 0) {
    improvements.push({
      priority: "high",
      action: `Investigate risk hotspot: ${riskHotspots[0].location}`,
      reason: `${riskHotspots[0].issueCount} issues concentrated in one location`,
    });
  }
  if (data.testCaseStats.draft > 0) {
    improvements.push({
      priority: "medium",
      action: `Review and execute ${data.testCaseStats.draft} draft test cases`,
      reason: "Untested scenarios may hide issues",
    });
  }
  if (!testedLayers.has("api") && data.testCaseStats.total > 0) {
    improvements.push({
      priority: "medium",
      action: "Add API layer test cases",
      reason:
        "Backend endpoints not directly tested — UI tests alone miss API-level bugs",
    });
  }
  if ((data.uiMapFlakyElements as unknown[]).length > 0) {
    improvements.push({
      priority: "medium",
      action: `Fix ${(data.uiMapFlakyElements as unknown[]).length} flaky UI selectors`,
      reason:
        "Flaky selectors cause false test failures and erode trust in results",
    });
  }
  if (unresolvedTech.length > 0) {
    improvements.push({
      priority: "medium",
      action: `Resolve ${unresolvedTech.length} tech issues`,
      reason:
        "Unresolved tech issues may be causing test failures or blocking execution",
    });
  }
  if ((data.analyses as unknown[]).length === 0) {
    improvements.push({
      priority: "low",
      action: "Run /noob-analyze before next test cycle",
      reason: "Impact analysis helps predict risk areas and focus testing",
    });
  }
  if (data.totalBlocked > 0) {
    improvements.push({
      priority: "high",
      action: `Unblock ${data.totalBlocked} blocked test${data.totalBlocked > 1 ? "s" : ""}`,
      reason:
        "Blocked tests represent untested functionality — often auth or environment issues",
    });
  }
  if (topCategory && topCategory[0] === "performance" && topCategory[1] >= 2) {
    improvements.push({
      ...
      reason: `${topCategory[1]} performance issues found — may indicate backend bottlenecks`,
    });
  }
  if (
    topCategory &&
    topCategory[0] === "accessibility" &&
    topCategory[1] >= 2
  ) {
    improvements.push({
      priority: "medium",
      action: "Address accessibility issues",
      reason: `${topCategory[1]} a11y issues — WCAG compliance at risk`,
    });
  }
