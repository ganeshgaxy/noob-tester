...
export function registerChainCommands(program: Command): void {
  ...
  program
    .command("init")
    .description(
      "Initialize session + run + run pack in one command (replaces 4 commands + 3 jq calls)",
    )
    ...
    .option(
      "--labels <labels>",
      "Session labels (comma-separated: analyze,plan,testcase,explore,report)",
    )
    .option(
      "--input-type <type>",
      "Input type: ticket, confluence, text, file (default: ticket)",
    )
    .option(
      "--secret-target <name>",
    ...
    .action((opts) => {
      ...
      if (!runId!) {
        ...
        db.prepare(
          ...
          opts.secretTarget ?? null,
          opts.secretRole ?? null,
        );
      }

      // 3. Session link
      db.prepare("UPDATE sessions SET current_run_id = ? WHERE id = ?").run(
        runId,
        sessionId,
      );
  ...
  program
    ...
    .option(
      "--status <status>",
      "Run status: completed, failed (default: completed)",
    )
  ...
  program
    ...
    .description(
      "Capture snapshot, screenshot, console, HAR for a page and register all artifacts",
    )
    .requiredOption("--run <runId>", "Run ID")
    ...
    .option(
      "--page-name <name>",
      "Page name for file naming (default: action-N)",
    )
    .option(
      "--map <mapId>",
      "UI map ID — if provided, also records page + scans elements",
    )
    .option("--page-title <title>", "Page title for UI map")
    .option(
      "--prev-page <pageId>",
      "Previous page ID — records navigation from prev to current",
    )
    .action((opts) => {
      ...
      if (captured.includes("snapshot"))
        storeArtifact("snapshot", snapshotPath);
      ...
      if (opts.entry && captured.length > 0) {
        ...
        addEntryLog(
          opts.entry,
          `[${opts.action}] ${desc} — ${captured.join(", ")} captured`,
        );
        ...
        if (axeRaw) {
          ...
          if (Array.isArray(violations) && violations.length > 0) {
            ...
            if (opts.entry && a11yCount > 0) {
              addEntryLog(
                opts.entry,
                `[${opts.action}] a11y: ${a11yCount} issue(s) — ${a11ySummary.map((v) => `${v.impact}: ${v.rule} (${v.nodes} elements)`).join(", ")}`,
              );
            }
          }
        }
      ...
      if (opts.map && captured.includes("snapshot")) {
        ...
        if (opts.prevPage && pageId) {
          ...
          const prevExists = db
            .prepare("SELECT id FROM ui_map_pages WHERE id = ?")
            .get(opts.prevPage);
          ...
          if (prevExists) {
            const existingNav = db
              ...
              .get(opts.map, opts.prevPage, pageId) as
              | { id: string }
              | undefined;
  ...
  program
    .command("claim-smart")
    .description(
      "Smart claim: claim pending entry, or pull next unclaimed test case. No auto-retry — use `runpack retry` for that.",
    )
    ...
    .action((opts) => {
      ...
      if (session && session.status !== "active") {
        console.log(
          JSON.stringify({
            ...
            hint: "Do not call claim-smart after ending the session. One test case per invocation.",
          }),
        );
        return;
      }
      ...
      const nameFilterPending = opts.name
        ? " AND LOWER(rpe.tc_title) LIKE LOWER(?)"
        : "";
      const pendingParams: unknown[] = [packId];
      if (opts.name) pendingParams.push(`%${opts.name}%`);
      const pending = db
        .prepare(
          `SELECT rpe.* FROM run_pack_entries rpe
         JOIN test_cases tc ON rpe.test_case_id = tc.id
         WHERE rpe.run_pack_id = ? AND rpe.status = 'pending' AND rpe.test_case_id != '__header__'${nameFilterPending}
         ORDER BY tc.priority ASC, rpe.created_at ASC LIMIT 1`,
        )
        .get(...pendingParams) as Record<string, unknown> | undefined;
      ...
      {
        const layerFilter = opts.layer
          ? " AND COALESCE(test_layer, 'ui') = ?"
          : "";
        const nameFilterNext = opts.name
        ...
        const allReady = db
          .prepare(
            `SELECT id FROM test_cases WHERE ticket_ref = ? AND ready = 1${layerFilter}${nameFilterNext} ${orderClause}`,
          )
  ...
  program
    ...
    .description(
      "Resolve credentials for a target+role (resolves 1Password refs, returns email/password/token)",
    )
    ...
    .action((opts) => {
      ...
      if (!targetName) {
        console.log(
          JSON.stringify({
            error:
              "No target specified. Use --target or --pack with stored secret_target.",
          }),
        );
        return;
      }
      ...
      try {
        ...
        const apiToken =
          ...
          null;
        const otpSecret =
          profile["OTP_SECRET"] ?? profile["LOGIN_OTP_SECRET"] ?? null;
        ...
        console.log(
          JSON.stringify({ error: String(err), target: targetName, role }),
        );
      }
    });
  ...
  if (repos) {
    repos
      ...
      .description(
        "Discover, sync, and index all repos for a ticket in one command",
      )
      .requiredOption("--ticket <id>", "Ticket ID")
      .option("--url <urls...>", "Additional repo URLs")
      .option(
        "--branch <branch>",
        "Switch all repos to this branch and reindex",
      )
      .action((opts) => {
        ...
        // From runs
        const runs = db
          .prepare("SELECT config_json FROM runs WHERE input_ref = ?")
          .all(opts.ticket) as Array<{ config_json: string }>;
        for (const r of runs) {
          try {
            const config = JSON.parse(r.config_json);
            if (config.repos)
              for (const url of config.repos) repoNames.add(url);
          } catch {}
        }

        // From all registered repos (they were discovered above)
        const allRepos = listRepos() as Array<{
          name: string;
        ...
        const results: Record<
          string,
          { synced: boolean; indexed: boolean; branch?: string }
        > = {};
  ...
  program
    .command("api-request")
    .description(
      "Execute an API request, validate, store artifact, and log result in one command",
    )
    .requiredOption(
      "--method <method>",
    ...
    .action((opts) => {
      ...
      const body =
        metaIdx >= 0 ? rawResponse.slice(0, metaIdx).trim() : rawResponse;
      ...
      if (opts.run) {
        const artifactPath = join(
          dir,
          `api-${opts.action ?? 0}-${method.toLowerCase()}.txt`,
        );
      ...
      if (opts.entry) {
        ...
        const entry = db
          ...
          .get(opts.entry) as { logs: string | null } | undefined;
        let logs: string[] = [];
        try {
      ...
      if (opts.apimapEndpoint) {
        ...
        const ep = db
          .prepare(
            "SELECT times_called, times_succeeded, times_failed, avg_response_ms FROM api_map_endpoints WHERE id = ?",
          )
          .get(opts.apimapEndpoint) as
          | {
              times_called: number;
        ...
        if (ep) {
          ...
          const newAvg =
            (ep.avg_response_ms * ep.times_called + timing * 1000) / newCalled;
          db.prepare(
