...
export function registerAuditCommands(testcaseCmd: Command): void {
  testcaseCmd
    .command("audit")
    .description("Audit test suite — find duplicates, never-failed, orphaned, and stale test cases")
    ...
    .option("--orphaned", "Only show test cases with no recent activity (across all tickets)")
    .option("--stale", "Only show test cases not executed in 30+ days")
    .option("--threshold <n>", "Similarity threshold for duplicates (0.0-1.0, default: 0.65)", parseFloat)
    .option("--json", "Output as JSON")
    .action((opts) => {
      // Single-mode shortcuts
      if (opts.duplicates) {
        const dupes = findDuplicates(opts.ticket, opts.threshold ?? 0.65);
        if (opts.json) { console.log(JSON.stringify(dupes, null, 2)); return; }
        printDuplicates(dupes);
        return;
      }
      if (opts.neverFailed) {
        const nf = findNeverFailed(opts.ticket);
        if (opts.json) { console.log(JSON.stringify(nf, null, 2)); return; }
        printNeverFailed(nf);
        return;
      }
      if (opts.orphaned) {
      ...
      if (opts.stale) {
        ...
        if (opts.json) { console.log(JSON.stringify(staleList, null, 2)); return; }
      ...
      console.log(`  Duplicates:        ${report.stats.duplicateCount > 0 ? chalk.yellow(String(report.stats.duplicateCount)) : chalk.green("0")} pairs`);
      console.log(`  Never failed:      ${report.stats.neverFailedCount > 0 ? chalk.yellow(String(report.stats.neverFailedCount)) : chalk.green("0")}`);
      console.log(`  Orphaned:          ${report.stats.orphanedCount > 0 ? chalk.yellow(String(report.stats.orphanedCount)) : chalk.green("0")}`);
      console.log(`  Stale (30+ days):  ${report.stats.staleCount > 0 ? chalk.yellow(String(report.stats.staleCount)) : chalk.green("0")}`);
...
function printDuplicates(dupes: Array<{ testCaseA: { title: string }; testCaseB: { title: string }; similarity: number }>) {
  console.log(chalk.bold("  Near-Duplicate Pairs:"));
  for (const d of dupes) {
    console.log(`    ${chalk.yellow(`${Math.round(d.similarity * 100)}%`)} ${d.testCaseA.title}`);
    console.log(`    ${chalk.dim("  ≈")} ${d.testCaseB.title}`);
  }
}

function printNeverFailed(items: Array<{ title: string; execution_count: number }>) {
  console.log(chalk.bold("  Never-Failed Test Cases:"));
  for (const nf of items) {
    console.log(`    ${chalk.dim(`${nf.execution_count}× executed`)} ${nf.title}`);
  }
}
