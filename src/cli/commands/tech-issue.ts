...
export function registerTechIssueCommands(program: Command): void {
  const ti = program
    .command("tech-issue")
    .description(
      "Manage technical issues encountered during testing (timeouts, crashes, env problems)",
    );
  ...
  ti.command("log <runId>")
    ...
    .option(
      "--category <cat>",
      "timeout|crash|network_failure|js_error|element_not_found|auth_issue|env_issue|unknown",
      "unknown",
    )
    ...
    .option(
      "--recovery <json>",
      "JSON array of recovery attempts [{attempt, result, duration_ms}]",
    )
    ...
    .action((runId, opts) => {
      ...
      const existing = db
        .prepare(
          "SELECT id, occurrence_count FROM tech_issues WHERE pattern_hash = ?",
        )
        .get(patternHash) as
  ...
  ti.command("resolve <id>")
    ...
    .requiredOption(
      "--status <status>",
      "unresolved|investigating|workaround_found|resolved|wont_fix",
    )
    .option(
      "--workaround <text>",
      "Workaround description (how to get past this)",
    )
  ...
  ti.command("check")
    .description(
      "Check for known tech issues at a URL or for a ticket (run before each step)",
    )
    ...
    .action((opts) => {
      ...
      sql +=
        " ORDER BY occurrence_count DESC, severity, updated_at DESC LIMIT 20";
  ...
  ti.command("list")
    ...
    .action((opts) => {
      ...
      const issues = db.prepare(sql).all(...params) as Array<
      ...
      for (const i of issues) {
        const statusColor =
          i.status === "resolved" || i.status === "workaround_found"
            ? chalk.green
        ...
        console.log(
          `  ${statusColor((i.status as string).padEnd(18))} ` +
            ...
            `${i.title}` +
            ((i.occurrence_count as number) > 1
              ? chalk.yellow(` (x${i.occurrence_count})`)
              : ""),
        );
        if (i.url) console.log(chalk.dim(`    @ ${i.url}`));
        if (i.workaround)
          console.log(chalk.green(`    Workaround: ${i.workaround}`));
