...
export function registerUiMapCommands(program: Command): void {
  const uimap = program
    .command("uimap")
    .description("Manage UI maps — persistent knowledge base of app UI structure, selectors, and navigation");

  // ── Map CRUD ──

  uimap.command("create")
    ...
    .action((opts) => {
      const id = createUiMap({
        ...
        repoUrls: opts.repos ? (opts.repos as string).split(",").map((s: string) => s.trim()) : undefined,
        targetUrls: opts.targets ? (opts.targets as string).split(",").map((s: string) => s.trim()) : undefined,
        ticketIds: opts.tickets ? (opts.tickets as string).split(",").map((s: string) => s.trim()) : undefined,
      });
      console.log(JSON.stringify({ uiMapId: id, name: opts.name }));
    });

  uimap.command("get <id>")
    .description("Get UI map details")
    .action((id) => {
      const map = getUiMap(id);
      if (!map) { console.error("UI map not found"); process.exit(1); }
  ...
  uimap.command("list")
    ...
    .action((opts) => {
      ...
      if (opts.json) { console.log(JSON.stringify(maps, null, 2)); return; }
      if (maps.length === 0) { console.log(chalk.dim("No UI maps.")); return; }
      ...
      for (const m of maps) {
        ...
        console.log(`    ${chalk.dim(`${stats.pages}pg ${stats.elements}el ${stats.navigations}nav ${stats.forms}form`)} ${chalk.green(`${stats.working}ok`)} ${chalk.yellow(`${stats.flaky}flaky`)} ${chalk.red(`${stats.broken}broken`)}`);
        if (repos.length) console.log(`    ${chalk.dim("repos:")} ${repos.join(", ")}`);
        if (targets.length) console.log(`    ${chalk.dim("targets:")} ${targets.join(", ")}`);
      }
      console.log();
    });

  uimap.command("resolve")
    ...
    .action((opts) => {
      const map = resolveUiMap({ ticketId: opts.ticket, repoUrl: opts.repo, targetUrl: opts.target });
      if (!map) { console.log(JSON.stringify({ found: false })); return; }
      const stats = getMapStats(map.id);
      console.log(JSON.stringify({ found: true, ...map, stats }, null, 2));
    });

  uimap.command("update <id>")
    ...
    .action((id, opts) => {
      updateUiMap(id, {
        name: opts.name,
        description: opts.description,
        addRepos: opts.addRepos ? (opts.addRepos as string).split(",").map((s: string) => s.trim()) : undefined,
        addTargets: opts.addTargets ? (opts.addTargets as string).split(",").map((s: string) => s.trim()) : undefined,
        addTicketIds: opts.addTickets ? (opts.addTickets as string).split(",").map((s: string) => s.trim()) : undefined,
      });
      console.log(JSON.stringify({ updated: true }));
    });

  uimap.command("delete <id>")
    .description("Delete a UI map and all its data")
    .option("--yes", "Skip confirmation")
    .action((id, opts) => {
      if (!opts.yes) { console.log(chalk.yellow("Run with --yes to confirm.")); return; }
      deleteUiMap(id);
  ...
  uimap.command("page <mapId>")
    .description("Record or update a page in the UI map (upserts by url)")
    .requiredOption("--url <pattern>", "URL pattern (e.g. /login, /dashboard/:id)")
    ...
    .option("--tickets <ids>", "Comma-separated ticket IDs")
    .option("--parity <json>", "Target parity JSON (e.g. {\"staging\":true,\"prod\":false})")
    ...
    .action((mapId, opts) => {
      const pageId = upsertPage({
        ...
        authRoles: opts.authRoles ? (opts.authRoles as string).split(",").map((s: string) => s.trim()) : undefined,
        relatedCode: opts.code ? (opts.code as string).split(",").map((s: string) => s.trim()) : undefined,
        relatedRepos: opts.repos ? (opts.repos as string).split(",").map((s: string) => s.trim()) : undefined,
        ticketIds: opts.tickets ? (opts.tickets as string).split(",").map((s: string) => s.trim()) : opts.ticket ? [opts.ticket] : undefined,
  ...
  uimap.command("pages <mapId>")
    .description("List pages in a UI map")
    .option("--json", "JSON output")
    .action((mapId, opts) => {
      const pages = listPages(mapId);
      if (opts.json) { console.log(JSON.stringify(pages, null, 2)); return; }
      if (pages.length === 0) { console.log(chalk.dim("No pages.")); return; }
      console.log(chalk.bold("\nPages\n"));
      for (const p of pages) {
        const statusColor = p.status === "active" ? chalk.green : p.status === "changed" ? chalk.yellow : chalk.red;
        const tickets = JSON.parse(p.ticket_ids) as string[];
        console.log(`  ${statusColor(p.status.padEnd(8))} ${chalk.cyan(p.url_pattern)} ${p.page_title || ""}`);
        if (tickets.length) console.log(`    ${chalk.dim("tickets:")} ${tickets.join(", ")}`);
      }
      console.log();
    });
  ...
  uimap.command("element <pageId>")
    .description("Record or update an element on a page (upserts by selector)")
    .requiredOption("--selector <sel>", "CSS selector")
    .requiredOption("--type <type>", "Element type: button|input|link|select|checkbox|radio|tab|menu|modal|text|image|other")
    .option("--role <role>", "ARIA role")
    .option("--text <text>", "Visible text/label")
    .option("--name <name>", "Form name/id")
    .option("--position <hint>", "Position hint: top-nav|sidebar|main|footer|modal|form|header")
    .option("--action <type>", "Action type: click|type|select|hover|scroll|toggle|submit")
    ...
    .option("--map <mapId>", "UI map ID (required if pageId is not directly under a map)")
    .action((pageId, opts) => {
      ...
      console.log(JSON.stringify({ elementId, selector: opts.selector, type: opts.type }));
    });

  uimap.command("elements <pageId>")
    .description("List elements on a page")
    .option("--json", "JSON output")
    .action((pageId, opts) => {
      const elements = listElements(pageId);
      if (opts.json) { console.log(JSON.stringify(elements, null, 2)); return; }
      if (elements.length === 0) { console.log(chalk.dim("No elements.")); return; }
      for (const e of elements) {
        const statusColor = e.status === "working" ? chalk.green : e.status === "flaky" ? chalk.yellow : chalk.red;
        const reliability = e.times_used > 0 ? Math.round(e.times_succeeded / e.times_used * 100) : 0;
        console.log(`  ${statusColor(e.status.padEnd(8))} ${chalk.cyan(e.element_type.padEnd(8))} ${e.selector.slice(0, 50)} ${e.element_text || ""} ${chalk.dim(`${reliability}% (${e.times_succeeded}/${e.times_used})`)}`);
      }
    });

  // ── Reliability ──

  uimap.command("hit <elementId>")
  ...
  uimap.command("flaky <mapId>")
    .description("List flaky and broken elements in a map")
    .option("--json", "JSON output")
    .action((mapId, opts) => {
      const elements = getFlakyElements(mapId);
      if (opts.json) { console.log(JSON.stringify(elements, null, 2)); return; }
      if (elements.length === 0) { console.log(chalk.green("No flaky or broken elements.")); return; }
      for (const e of elements) {
        const statusColor = e.status === "flaky" ? chalk.yellow : chalk.red;
        const reliability = e.times_used > 0 ? Math.round(e.times_succeeded / e.times_used * 100) : 0;
        console.log(`  ${statusColor(e.status.padEnd(8))} ${e.selector.slice(0, 50)} ${chalk.dim(`${reliability}% (${e.times_failed} failures)`)}`);
      }
    });

  // ── Lookup ──

  uimap.command("lookup")
    ...
    .action((opts) => {
      ...
      if (opts.json) { console.log(JSON.stringify(elements, null, 2)); return; }
      if (elements.length === 0) { console.log(chalk.dim(`No elements found for ${opts.url}`)); return; }
      console.log(chalk.bold(`\nElements at ${opts.url}\n`));
      for (const e of elements) {
        const statusColor = e.status === "working" ? chalk.green : e.status === "flaky" ? chalk.yellow : chalk.red;
        const reliability = e.times_used > 0 ? `${Math.round(e.times_succeeded / e.times_used * 100)}%` : "new";
        console.log(`  ${statusColor(e.status.padEnd(8))} ${chalk.cyan(e.element_type.padEnd(8))} ${e.selector}`);
        console.log(`    ${e.element_text || ""} ${e.action_type ? `→ ${e.action_type}` : ""} ${e.action_result ? `→ ${e.action_result}` : ""} ${chalk.dim(reliability)}`);
      }
      console.log();
    });
  ...
  uimap.command("nav <mapId>")
    ...
    .option("--type <navType>", "Navigation type: click|redirect|form_submit|url_change", "click")
  ...
  uimap.command("path")
    ...
    .action((opts) => {
      ...
      if (!path) {
        console.log(JSON.stringify({ found: false, message: `No path from ${opts.from} to ${opts.to}` }));
        return;
      }
      console.log(JSON.stringify({ found: true, steps: path.length, path: path.map(s => ({
        url: s.page.url_pattern,
        title: s.page.page_title,
        via: s.element ? { selector: s.element.selector, text: s.element.element_text, action: s.element.action_type } : null,
  ...
  uimap.command("form <pageId>")
    ...
    .action((pageId, opts) => {
      ...
      const formId = upsertForm({
        ...
        sampleValues: opts.sampleValues ? JSON.parse(opts.sampleValues) : undefined,
        ticketIds: opts.tickets ? (opts.tickets as string).split(",").map((s: string) => s.trim()) : opts.ticket ? [opts.ticket] : undefined,
        createdByRun: opts.run,
        createdByTicket: opts.ticket,
      });
  ...
  uimap.command("scan <pageId>")
    .description("Parse an accessibility snapshot and bulk-record all elements + forms on a page")
    ...
    .action((pageId, opts) => {
      ...
      const ticketIds = opts.tickets
        ...
        : opts.ticket ? [opts.ticket] : undefined;
