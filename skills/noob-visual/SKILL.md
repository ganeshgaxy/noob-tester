  ...
  for ((i = 0; i < STEP_COUNT; i++)); do
    ...
    case "$S_ACTION" in
      ...
      click)
        # ╔══════════════════════════════════════════════════════════════════╗
        # ║ ELEMENT DISCOVERY                                               ║
        # ║ The step description tells you WHAT to click.                  ║
        # ║ Read the snapshot from the PREVIOUS capture to find the @eN    ║
        # ║ ref for this element. Use that ref to click — don't guess.     ║
        # ╚══════════════════════════════════════════════════════════════════╝
        if [ -n "$S_SELECTOR" ] && [ "$S_SELECTOR" != "null" ]; then
          agent-browser click "$S_SELECTOR"
        else
          # Read last snapshot to find the element ref for: $S_DESC
          # Then: agent-browser click '@eN'
          echo "DISCOVER: find ref for '$S_DESC' in last snapshot, then click"
        fi
        ;;
      fill)
        if [ -n "$S_SELECTOR" ] && [ "$S_SELECTOR" != "null" ]; then
